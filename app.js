...
function renderRollingChart(slice, windowSize) {
  ...
  function fmtYtick(v){return unit==='woba'?'.'+Math.round(v*1000):unit==='pct'?Math.round(v)+'%':unit==='in'?v.toFixed(1)+'"':Math.round(v)+'\u00b0';}
...
function ppQuadMobile(X,Y,rows,y25,y26){
  ...
  var svg='<svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto;max-height:100%;display:block;margin:0 auto">'
   ...
   +'<rect x="'+ixL+'" y="'+iyT+'" width="'+ixW+'" height="'+iyH+'" fill="#e0a878" fill-opacity="0.06"/>'
   +cloud
...
function ppDrawRolling(){
  ...
  var fmtYtick=v=>unit==='woba'?'.'+Math.round(v*1000):unit==='pct'?Math.round(v)+'%':unit==='in'?v.toFixed(1)+'"':Math.round(v)+'\u00b0';
  ...
  var s='';
  var fmtLeg=v=>unit==='woba'?('.'+Math.round(v*1000)):unit==='pct'?Math.round(v)+'%':unit==='in'?v.toFixed(1)+'"':Math.round(v)+'\u00b0';
  if(mob){
...
(function(){
  ...
  function rollChart(w,h){
    ...
    function fmt(v){return isW?f3(v):cfg.unit==='pct'?Math.round(v)+'%':cfg.unit==='deg'?Math.round(v)+'\u00b0':cfg.unit==='in'?v.toFixed(1)+'"':Math.round(v);}
    ...
    s+='<text x="'+mL+'" y="19" font-family="Inter" font-size="16"><tspan fill="#f5f5f0" font-weight="800">Last '+N+' PA</tspan><tspan fill="#cfcfc9" font-weight="600"> \u00b7 '+cfg.label+'</tspan></text>';
    var lx=mL;
    ...
    s+='</svg>';
    _boxGeom={lines:L.map(function(o){return {name:o.lab,color:o.c,v:o.vals};}),
              dates:dts, len:len, win:N, mob:false, vbW:w,
              mL:mL, iw:iw, mT:mT, ih:ih, X:X, Y:Y, fmtVal:fmt};
    return tabBar()+s;
  }
  var _boxGeom=null;
  ...
  function place(){
    ...
    if(_boxGeom)wireRollTip(box,_boxGeom);
